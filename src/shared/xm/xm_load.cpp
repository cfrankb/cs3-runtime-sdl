...
#define _u8p(x) reinterpret_cast<uint8_t*>(x)
#define _u8pc(x) reinterpret_cast<const uint8_t*>(x)
 ...
 */
#define READ_U8_BOUND(offset, bound) static_cast<xm_waveform_type_t>( (((offset) < (bound)) ? (*(uint8_t*)(moddata + (offset))) : 0) )
...
static inline void memcpy_pad(void* dst, size_t dst_len, const void* src, size_t src_len, size_t offset) {
	uint8_t* dst_c = _u8p(dst);
	const uint8_t* src_c = _u8pc(src);
...
char* xm_load_module(xm_context_t* ctx, const char* moddata, size_t moddata_length, char* mempool) {
	...
	for(uint16_t i = 0; i < ctx->module.num_instruments; ++i) {
		...
		if(instr->num_samples > 0) {
			...
			instr->vibrato_type = static_cast<xm_waveform_type_t>(READ_U8_BOUND(offset + 235, offset + ins_header_size));
			if(instr->vibrato_type == 2) {
				instr->vibrato_type = static_cast<xm_waveform_type_t>(1);
			} else if(instr->vibrato_type == 1) {
				instr->vibrato_type = static_cast<xm_waveform_type_t>(2);
			}
