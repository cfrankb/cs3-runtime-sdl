...
target_include_directories(${PROJECT_NAME} PRIVATE
    ...
    '    COMMAND "${CMAKE_STRIP}" "$<TARGET_FILE:APP_NAME>"'.replace(
        "APP_NAME", APP_NAME
    ),
    ...
    "",
    "execute_process(",
    '    COMMAND gzip -9 -c "${CMAKE_SOURCE_DIR}/packages/data/debian/APP_NAME.6"'.replace(
        "APP_NAME", APP_NAME
    ),
    '    OUTPUT_FILE "${CMAKE_BINARY_DIR}/APP_NAME.6.gz"'.replace("APP_NAME", APP_NAME),
    ")",
    'install(FILES "${CMAKE_BINARY_DIR}/APP_NAME.6.gz"'.replace("APP_NAME", APP_NAME),
