...
#idx_music = {}
#with open('packages/data/musics.json', 'w', encoding='utf-8') as f:
#    json.dump(musics, f, ensure_ascii=False, indent=4)
...
columns = ['','Component Name', 'Version', 'License', 'SPDX ID', 'Source URL', 'Attribution Required','']
...
with open('packages/data/components.json', 'r') as file:
    ...
    for item in data:
        ...
        row = ['', item['name'], item['version'], item['license'], item['SPDX_ID'], source, item['attribution'], '']
...
with open('packages/data/musics.json', 'r') as file:
    ...
    for item in data:
        if item['name'] in removed:
            continue
        if item['name'] not in config['musics']:
